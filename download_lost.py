...
def get_download_list(lost_names, subset_opt: str, hash_name: str, reso_opt: str, file_type: str, output_dir: str):
    ...
    for ln in lost_names:
        ret.append(to_download_item(ln, reso_opt, subset_opt, file_type))
...
def download(download_list: list, output_dir: str, is_clean_cache: bool):
    ...
    for item in tqdm(download_list, desc='Downloading'):
        ...
        save_file_path = join(output_dir, rel_path)
        if os.path.exists(save_file_path):
            print('rm download file.')
...
def download_dataset(args):
    ...
    for subset_opt in ['1K', '5K', '8K', '10K', '11K']:
        lost_names = get_lostnames(subset_opt)
        download_list = get_download_list(lost_names, subset_opt, hash_name, reso_opt, file_type, output_dir)
        download_res = download(download_list, output_dir, is_clean_cache)
        if download_res == False:
            print('cannot download all', subset_opt)
    
    return True

def get_lostnames(subset_opt):
    fp = f'/home/szxie/DL3DV/lost_zips/{subset_opt}.txt'
    lost_names = []
    with open(fp) as f:
        lines = f.readlines()
        for line in lines:
            lost_names.append(line.strip())
